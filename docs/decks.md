# The Three Decks

---

## Overview

QuestPay™ requires different messaging for different audiences. We've developed three specialized pitch decks, each tailored to specific stakeholder priorities.

---

## Sponsor Edition

### Target Audience

- Banks and digital wallets
- Major merchants and retailers
- FMCG and consumer brands
- Telecommunications companies

### Key Emphasis

**Redemption Engagement & Brand ROI**

- Redemption attribution and measurement
- User acquisition metrics
- Brand exposure quantification
- Social media amplification
- Competitive positioning

### Core Messages

1. **Measurable Impact**: Direct attribution of redemptions to show participation
2. **Youth Engagement**: Access to digitally-savvy demographic
3. **Innovation Association**: Position brand as forward-thinking via redemption mechanics
4. **Content Marketing**: Authentic brand integration vs. traditional ads
5. **Redemption Insights**: User behavior and preference data through voucher analytics

### Deck Structure

1. Market opportunity
2. Format overview (2-3 slides)
3. Sponsorship packages and pricing
4. Brand integration examples
5. ROI metrics and measurement
6. Success stories / case studies
7. Next steps and timeline

**Length**: 15-20 slides  
**Tone**: Business-focused, ROI-driven

---

## Ownership Edition

### Target Audience

- Celebrities and influencers
- Media producers and production companies
- Investors and venture capitalists
- Entertainment industry executives

### Key Emphasis

**Format Rights, Equity & Franchise Model**

- IP ownership and protection
- Revenue share structures
- Franchise licensing potential
- Equity participation options
- Celebrity/influencer involvement opportunities

### Core Messages

1. **Proven Format**: Reality competition + redemption mechanics = tested formula
2. **Scalable IP**: Franchiseable redemption mechanics across markets and themes
3. **Multiple Revenue Streams**: Not just ad-dependent
4. **Celebrity Opportunities**: Host, judge, or producer roles
5. **Growth Potential**: Pilot → Season → Franchise → Global

### Deck Structure

1. Format overview and hook
2. Market size and opportunity
3. Competitive landscape
4. IP and ownership structure
5. Revenue model and projections
6. Franchise roadmap
7. Team and track record
8. Investment ask and use of funds

**Length**: 20-25 slides  
**Tone**: Investment-pitch style, growth-focused

---

## Tech-Backbone Edition

### Target Audience

- Fintech partners and wallet providers
- Internal R&D and engineering teams
- Technology vendors and platforms
- Regulatory and compliance teams

### Key Emphasis

**Architecture, API Flow & Data Integrity**

- System architecture and scalability
- API integration requirements
- Security and compliance
- Data flow and verification
- Technical feasibility

### Core Messages

1. **Built on x-Change**: Proven redemption platform
2. **API-First Design**: Easy integration with existing wallets
3. **Real-Time Verification**: Instant redemption validation
4. **Compliance-Ready**: KYC/AML built-in via redemption wizard
5. **Scalable Architecture**: Handle 100k+ concurrent redemptions

### Deck Structure

1. System architecture overview
2. Core components and technology stack
3. API integration guide
4. Security and compliance framework
5. Data flow diagrams
6. Scalability and performance
7. Development roadmap
8. Integration timeline and support

**Length**: 25-30 slides  
**Tone**: Technical, detailed, architecture-focused

---

## Cross-Deck Consistency

### Shared Elements

All three decks share:

- QuestPay™ brand identity and tagline: **"Redeem. Race. Repeat."**
- Core concept explanation
- Sample gameplay scenarios
- Market validation data
- Team credentials
- Contact information

### Customized Elements

Each deck customizes:

- Value proposition
- Success metrics
- Risk/opportunity framing
- Financial projections
- Call to action

---

## Supporting Materials

### One-Pagers

- Executive summary
- Sponsor package overview
- Technical integration brief

### Videos

- 60-second concept teaser
- 3-minute format explainer
- 5-minute pilot episode preview

### Documents

- Full format bible
- Technical specification
- Sample episode scripts
- Contestant agreement templates

---

## Usage Guidelines

### Sponsor Edition

**When to Use**:
- Initial sponsor outreach
- Brand partnership meetings
- Media buying presentations

**Who Presents**:
- Business development
- Marketing director
- Show creator

### Ownership Edition

**When to Use**:
- Celebrity/talent pitches
- Investor roadshows
- Production partner meetings

**Who Presents**:
- Executive producer
- Show creator
- CFO/financial lead

### Tech-Backbone Edition

**When to Use**:
- Wallet provider meetings
- Technical due diligence
- Engineering team briefings

**Who Presents**:
- Technical director
- CTO/lead engineer
- Compliance officer

---

## Deck Maintenance

### Update Frequency

- **Quarterly**: Market data, projections
- **Per pitch**: Customize for specific audience
- **Post-pilot**: Add real results and metrics

### Version Control

- Master deck repository
- Dated versions
- Customization tracking
- Approved messaging guidelines

---

[← Back to Production](production.md) | [Continue to IP & Licensing →](licensing.md)
