...
```mermaid
graph TD
    A[Digital Ad] -->|Click| B[Registration Form]
    B -->|Submit| C[SMS: First Voucher Code]
    C -->|User Clicks Link| D[x-Change Redemption Page]
    D -->|Enter Code| E[Validate Voucher]
    E -->|Valid| F[Landing Page: Data Collection]
    F -->|Submit Info| G[Verify & Credit Pesos]
    G -->|Confirmed| H[Splash Screen: Sponsor Ad]
    H -->|Continue| I[Next Landing Page: Challenge + Voucher]
    I -->|User Completes| D
    D -->|Repeat Loop| J[Progressive Challenges]
    J -->|Final Voucher| K[On-Site Finale]
    K -->|Winner| L[Grand Prize Awarded]
